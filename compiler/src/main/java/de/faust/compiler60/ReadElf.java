/*
 * From: https://cs.android.com/android/platform/superproject/+/master:platform_testing/libraries/compatibility-common-util/src/com/android/compatibility/common/util/ReadElf.java
 * Modifications from FAUST: Replace RandomAccessFile with ByteBuffer
 * 
...
public class ReadElf {
  ...
  private final ByteBuffer mFile;
   ...
   * Rodata offset
   */
  public long mRodataOffset;

  /**
   * Rodata size
   */
  public int mRodataSize;
  ...
  public boolean isElf() {
    try {
      if (mFile.capacity() < EI_NIDENT) {
        throw new IllegalArgumentException(
                "Too small to be an ELF file");
      }
      
      byte[] buffer = new byte[512];
      mFile.get(0, buffer, 0, EI_NIDENT);
  ...
  public ReadElf(ByteBuffer byteBuffer) throws IOException {
    mHasRodata = false;
    mRoData = null;
    mFile = byteBuffer;

    if (mFile.capacity() < EI_NIDENT) {
      throw new IllegalArgumentException("Too small to be an ELF file");
    }
  ...
  private void readHeader() throws IOException {
    ...
    if (mEndian == ELFDATA2LSB) {
      ...
      throw new IOException("Unsupported ELFDATA2MSB file");
    } else {
      throw new IOException("Invalid ELF EI_DATA: " + mEndian);
    }
    ...
    if (e_version != EV_CURRENT) {
      throw new IOException("Invalid e_version: " + e_version);
    }
  ...
  private void readSectionHeaders(long sh_off, int e_shnum, int e_shentsize, int e_shstrndx)
    ...
    {
      mFile.position(Math.toIntExact(sh_off) + e_shstrndx * e_shentsize);
    ...
    for (int i = 0; i < e_shnum; ++i) {
      ...
      mFile.position(Math.toIntExact(sh_off) + i * e_shentsize);
  ...
  private void readProgramHeaders(long ph_off, int e_phnum, int e_phentsize) throws IOException {
    for (int i = 0; i < e_phnum; ++i) {
      mFile.position(Math.toIntExact(ph_off) + i * e_phentsize);
  ...
  private HashMap<String, Symbol> readSymbolTable(
    ...
    mFile.position(Math.toIntExact(tableOffset));
    int i = 0;
    while (mFile.position() < tableOffset + tableSize) {
  ...
  private int[] getVerSym() throws IOException {
    if (mVerSym == null) {
      mFile.position(Math.toIntExact(mVerSymTabOffset));
  ...
  private VerNeed[] getVerNeedArr() throws IOException {
    if (mVerNeedArr == null) {
      ...
      for (int i = 2; i < mVerNeedEntryCnt + 2; i++) {
        mFile.position(Math.toIntExact(idx));
        mVerNeedArr[i] =
        ...
        for (int j = 0; j < mVerNeedArr[i].vn_cnt; j++) {
          mFile.position(Math.toIntExact(idxAux));
  ...
  private String readString(long offset) throws IOException {
    mFile.get(Math.toIntExact(offset), 
            mBuffer, 0, (int) Math.min(mBuffer.length, mFile.capacity() - offset));
  ...
  private List<DynamicEntry> getDynamicList() throws IOException {
    if (mDynamicArr == null) {
      ...
      mFile.position(Math.toIntExact(mDynamicTabOffset));
      System.out.println(
              String.format(
  ...
  public byte[] getRoData() throws IOException {
    if (mHasRodata && mRoData == null) {
      ...
      mFile.position(Math.toIntExact(mRodataOffset));
      mFile.get(mRoData);
    }
